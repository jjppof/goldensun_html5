...
export class ItemPsynergyChooseWindow {
  ...
  constructor(
    ...
    this.element_list = this.is_psynergy_window
      ? this.data.info.abilities_list
      : this.data.info.items_list;
    this.element_sprite_key = this.is_psynergy_window
      ? "abilities_icons"
      : "items_icons";
    ...
    const sprite_pair = this.window.set_text_in_position(
      ": Change Char",
      49,
      96
    );
    this.char_select_controls_sprites.push(
      sprite_pair.text,
      sprite_pair.shadow
    );
  ...
  next_page() {
    ...
    if (this.page_index < this.page_number - 1)
      this.page_change(this.page_index + 1);
  ...
  get_element_key_name(index: number) {
    return this.is_psynergy_window
      ? this.elements[index]
      : (this.elements[index] as ItemSlot).key_name;
  }
  ...
  set_page_number() {
    ...
    this.window.page_indicator.initialize(this.page_number);
  }
  ...
  set_elements() {
    ...
    if (this.selected_element_index >= this.elements.length) {
      ...
      this.move_cursor(
        CURSOR_X,
        CURSOR_Y + this.selected_element_index * CURSOR_GAP
      );
    }
      ...
      if (this.is_psynergy_window) {
        this.icon_sprites_in_window.push(
          this.window.create_at_group(
            icon_x,
            icon_y,
            this.element_sprite_key,
            undefined,
            elem_key_name
          )
        );
        (this.icon_sprites_in_window[i] as Phaser.Sprite).anchor.setTo(
          0.5,
          0.5
        );
      } else {
        let icon_group = this.game.add.group();
        let icon_sprite = icon_group.create(
          0,
          0,
          this.element_sprite_key,
  ...
  set_highlight_bar() {
    ...
    this.highlight_bar.y =
      ...
      this.selected_element_index *
        (numbers.ICON_HEIGHT + SPACE_BETWEEN_ITEMS) +
      4;
  }
  ...
  unset_element_tween(index: number) {
    if (this.icon_sprites_in_window[index])
      this.icon_sprites_in_window[index].scale.setTo(1, 1);
  ...
  page_change(page: number) {
    ...
    this.on_change(
      this.element_list[
        this.get_element_key_name(this.selected_element_index) as string
      ],
      this.is_psynergy_window
        ? undefined
        : this.item_objs[this.selected_element_index]
    );
    this.window.page_indicator.set_highlight(this.page_index);
  }
