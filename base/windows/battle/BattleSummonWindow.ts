...
export class BattleSummonWindow {
  ...
  previous_summon() {
    if (this.summons.length === 1) return;
    this.select_summon(
      (this.summon_index + this.summons.length - 1) % this.summons.length
    );
  }
  ...
  change_page() {
    ...
    if (this.set_description) {
      this.set_description(
        this.data.info.abilities_list[this.summons[this.summon_index].key_name]
          .description
      );
    }
    ...
    this.djinn_numbers_window.set_numbers(
      this.summons[this.summon_index].requirements
    );
  }
  ...
  set_highlight_bar() {
    this.highlight_bar.y =
      TOP_PADDING +
      this.summon_index * (SPACE_BETWEEN_ITEMS + HIGHLIGHT_BAR_HEIGHT);
  }

  config_page() {
    this.clear_sprites();
    this.summons = this.all_summons.slice(
  ...
  mount_window() {
    const standby_djinni = Djinn.get_standby_djinni(
      ...
      MainChar.get_active_players(
        this.data.info.party_data,
        Battle.MAX_CHARS_IN_BATTLE
      )
    );
    ...
    this.all_summons = _.flatMap(this.data.info.summons_list, (summon) => {
      ...
      const can_be_summoned = _.every(
        summon.requirements,
        (value, elem) => value <= standby_djinni[elem]
      );
      return [
