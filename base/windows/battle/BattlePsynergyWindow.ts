...
export class BattlePsynergyWindow {
  ...
  previous_ability() {
    if (this.abilities.length === 1) return;
    this.select_ability(
      (this.ability_index + this.abilities.length - 1) % this.abilities.length
    );
  }
  ...
  change_page() {
    ...
    if (this.set_description) {
      this.set_description(
        this.data.info.abilities_list[this.abilities[this.ability_index]]
          .description
      );
    }
  ...
  set_highlight_bar() {
    this.highlight_bar.y =
      ELEM_PADDING_TOP +
      this.ability_index * (numbers.ICON_HEIGHT + SPACE_BETWEEN_ITEMS) +
      4;
  }
  ...
  set_abilities_list() {
    ...
    for (let i = 0; i < this.abilities.length; ++i) {
      ...
      if (this.psy_sealed) {
        ...
        this.char.current_pp < this.data.info.abilities_list[key_name].pp_cost
      ) {
