...
export class Window {
  ...
  set_text(
    ...
    const x_pos =
      padding_x === undefined ? numbers.WINDOW_PADDING_H + 4 : padding_x;
    let y_pos =
      padding_y === undefined
        ? numbers.WINDOW_PADDING_TOP + top_shift
        : padding_y;
    ...
    for (let i = 0; i < lines.length; ++i) {
      let line = lines[i];
      let text_sprite = this.game.add.bitmapText(
        x_pos,
        y_pos,
        font_name,
        animate ? "" : line,
  ...
  set_text_in_position(
    ...
    if (with_bg) {
      ...
      if (
        internal_group_key === undefined ||
        !this.add_to_internal_group(internal_group_key, text_bg)
      ) {
  ...
  update_text_position(new_position, text_shadow_pair) {
    ...
    if (text_shadow_pair.right_align) {
      text_shadow_pair.text.x =
        text_shadow_pair.initial_x - text_shadow_pair.text.width;
      text_shadow_pair.shadow.x =
        text_shadow_pair.initial_x - text_shadow_pair.shadow.width + 1;
      if (text_shadow_pair.text_bg) {
        text_shadow_pair.text_bg.x = text_shadow_pair.text.x - 1;
      }
    }
